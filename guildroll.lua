...
function GuildRoll:buildMenu()
  if not (options) then
    ...
    -- Import CSV - loot manager only (RollWithEP permission)
    options.args["quick_actions"].args["import_csv"] = {
      type = "execute",
      name = L["Import CSV"],
      desc = "Import soft reserves from RaidRes CSV",
      order = 6,
      hidden = function()
        ...
        StaticPopupDialogs["ROLLWITHEP_IMPORT_CSV"] = {
          ...
          OnAccept = function()
            ...
            if text and text ~= "" then
              ...
              StaticPopupDialogs["ROLLWITHEP_IMPORT_CSV_CONFIRM"] = {
                text = "Import will replace current SR data. Continue?",
                button1 = "Confirm",
                button2 = "Cancel",
                OnAccept = function()
                  if GuildRoll and GuildRoll.RollWithEP_ImportCSV then
                    GuildRoll.RollWithEP_ImportCSV(text)
                  end
                end,
                timeout = 0,
                whileDead = true,
              ...
              StaticPopup_Show("ROLLWITHEP_IMPORT_CSV_CONFIRM")
            end
          end,
          timeout = 0,
          whileDead = true,
    ...
    -- Set DE/Bank - loot manager only (RollWithEP permission)
    options.args["quick_actions"].args["set_de_bank"] = {
      type = "execute",
      name = L["Set DE/Bank"],
      desc = "Set player for DE/Bank items",
      order = 7,
      hidden = function()
        -- Only visible to loot manager (admin + ML/RL)
        if not GuildRoll or not GuildRoll.RollWithEP_CanUse then
      ...
      func = function()
        ...
        if numRaid > 0 then
          ...
          if numParty > 0 then
            ...
            table.insert(raidMembers, string.gsub(playerName, "%-[^%-]+$", ""))
            for i = 1, numParty do
              local name = UnitName("party" .. i)
              if name then
                table.insert(raidMembers, string.gsub(name, "%-[^%-]+$", ""))
              end
            end
          end
        end
        ...
        pcall(function()
          ...
          D:Open(UIParent,
            "children", function()
              ...
              if GuildRoll and GuildRoll.SafeDewdropAddLine then
                GuildRoll:SafeDewdropAddLine(
                  "text", "Clear (use Master Looter)",
                  "func", function()
                    GuildRoll.VARS.lootDE = nil
                    GuildRoll.VARS.lootDE_ML = nil
                    if GuildRoll and GuildRoll.defaultPrint then
                      GuildRoll:defaultPrint("DE/Bank player cleared. Using Master Looter.")
                    end
                    ...
                    "func", function()
                      ...
                      if method == "master" then
                        if GetNumRaidMembers() > 0 and raidIndex then
                          currentML = string.gsub(UnitName("raid" .. raidIndex) or "", "%-[^%-]+$", "")
                        elseif GetNumPartyMembers() > 0 and partyIndex then
                          if partyIndex == 0 then
                            currentML = string.gsub(UnitName("player") or "", "%-[^%-]+$", "")
                          else
                            currentML = string.gsub(UnitName("party" .. partyIndex) or "", "%-[^%-]+$", "")
                          end
                        end
                      end
                      ...
                      if GuildRoll and GuildRoll.defaultPrint then
                        GuildRoll:defaultPrint(string.format("DE/Bank player set to: %s", name))
                      end
                      pcall(function() D:Close() end)
                    end
                  )
