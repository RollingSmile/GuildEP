...
-- Defensive: Store local references to string functions to protect against corruption by other addons
-- This fixes a pre-existing bug where another addon corrupts the global string table
local string_match = string and string.match or nil
local string_gmatch = string and string.gmatch or nil
local string_gsub = string and string.gsub or nil
local string_find = string and string.find or nil
local string_format = string and string.format or nil
local string_len = string and string.len or nil
local string_sub = string and string.sub or nil

-- If string functions are corrupted at load time, show a one-time warning and create fallback wrappers
if not string_match or not string_gsub or not string_format then
  local warned = false
  if not warned then
    warned = true
    ...
    f:SetScript("OnEvent", function()
      DEFAULT_CHAT_FRAME:AddMessage("|cffff9900GuildRoll: String table corrupted by another addon. Functionality may be limited.|r")
