...
-- API: Import CSV data from RaidRes
-- CSV format expected: PlayerName,ItemID,ItemName,Weeks (for CSR)
-- Example: "Playername,12345,Item Name,2" or "Playername,12345,Item Name" for SR
-- TODO: Full RollTable UI, tie logic, and GiveToMember/GiveSR/GiveToDE delivery in follow-up PR
function GuildRoll.RollWithEP_ImportCSV(text)
  if not CanUseRollWithEP() then
    if GuildRoll and GuildRoll.defaultPrint then
      GuildRoll:defaultPrint(L["Only master looter or raid leader admin can use this feature."])
    end
  ...
  if not text or text == "" then
    if GuildRoll and GuildRoll.defaultPrint then
      GuildRoll:defaultPrint("Import failed: No data provided")
    end
    return false
  end
  
  -- Initialize RollForEP if needed
  if not RollForEP then
    RollForEP = {}
  end
  if not RollForEP.srData then
    RollForEP.srData = {}
  end
  
  -- Parse CSV with defensive pcall
  local success, result = pcall(function()
    ...
    for line in string.gmatch(text, "[^\r\n]+") do
      ...
      if lineCount == 1 and (string.match(line, "^[Pp]layer") or string.match(line, "^[Nn]ame")) then
        -- Skip header
      else
        -- Parse CSV line: PlayerName,ItemID,ItemName,Weeks
        local parts = {}
        ...
        if table.getn(parts) >= 3 then
          ...
          local weeks = parts[4] or nil
          
          -- Sanitize player name
          playerName = StripRealm(playerName)
          if playerName and playerName ~= "" then
            -- Initialize player entry
            if not newSRData[playerName] then
              newSRData[playerName] = {
                sr = 0,
                weeks = 0,
                items = {}
              }
            ...
            -- Add weeks if CSR
            if weeks and tonumber(weeks) and tonumber(weeks) > 0 then
              item.weeks = tonumber(weeks)
              newSRData[playerName].weeks = newSRData[playerName].weeks + tonumber(weeks)
            else
              newSRData[playerName].sr = newSRData[playerName].sr + 1
            end
            
            table.insert(newSRData[playerName].items, item)
          else
    ...
    return newSRData, lineCount, errorCount
  end)
  
  if not success or not result then
    if GuildRoll and GuildRoll.defaultPrint then
      GuildRoll:defaultPrint("Import failed: Parse error")
    end
    return false
  end
  
  local newSRData, lineCount, errorCount = result, select(2, pcall(function() return result end)), select(3, pcall(function() return result end))
  
  -- Store in RollForEP.srData
  RollForEP.srData = newSRData
  ...
  if GuildRoll and GuildRoll.defaultPrint then
    local playerCount = 0
    for _ in pairs(newSRData) do
      playerCount = playerCount + 1
    end
    GuildRoll:defaultPrint(string.format("CSV import complete: %d players, %d lines processed", playerCount, lineCount - 1))
    if errorCount > 0 then
      GuildRoll:defaultPrint(string.format("Warning: %d lines had errors", errorCount))
    end
  end
